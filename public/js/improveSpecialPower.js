...
document.querySelector("#activateSpecialPowerButton").addEventListener('click' , improveSpecialPowerFunc)

async function improveSpecialPowerFunc(event) {
    ...
    if (accounts.length) {
        ...
        if (playerXP < 250) {
            ...
            const txn = await contract.getSpecialPower();
            await txn.wait();
